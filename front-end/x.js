document.addEventListener("DOMContentLoaded", async () => {
    ...
    const getCheckedValues = (className) =>
        Array.from(document.querySelectorAll(`.${className}:checked`)).map(cb => cb.value);

    // Treatment Notes
    const notes = Array.from(document.querySelectorAll("#notesBody tr")).map(tr => ({
        note: tr.querySelector("input[type='text']").value,
        teethInvolved: Array.from(tr.querySelectorAll("td:nth-child(2) input[type='checkbox']:checked")).map(cb => cb.value),
        date: tr.querySelector("input[type='date']").value
    }));

    // Drug History
    const drugs = Array.from(document.querySelectorAll("#drugTable tbody tr")).map(tr => ({
        medicineName: tr.querySelector(".medName").value,
        dosage: tr.querySelector(".dosage").value,
        duration: tr.querySelector(".duration-select").value,
        instructions: tr.querySelector(".instructions").value
    }));

    // Reports (just names, you can extend to upload files)
    const reports = Array.from(document.querySelectorAll("#reportContainer .report-card span")).map(span => span.textContent);

    // Full Data Object
    return {
        patientId,
        cc: getCheckedValues("cc-checkbox"),
        ho: getCheckedValues("ho-checkbox"),
        ss: getCheckedValues("ss-checkbox"),
        dx: getCheckedValues("dx-checkbox"),
        tp: getCheckedValues("tp-checkbox"),
        io: getCheckedValues("io-checkbox"),
...
const logoHeight = 35;
// Horizontal center
const logoX = (doc.internal.pageSize.getWidth() - logoWidth) / 2 +25;
// Vertical center in header
